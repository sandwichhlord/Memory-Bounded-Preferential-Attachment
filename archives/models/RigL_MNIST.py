...
def get_dataloaders():
    transform = transforms.Compose([
        transforms.ToTensor(),
        # FIX 1: MNIST 1-channel normalization
        transforms.Normalize((0.1307,), (0.3081,)) 
    ])
    # FIX 2: Download true MNIST
    train_data = datasets.MNIST('./data', train=True, download=True, transform=transform)
    test_data = datasets.MNIST('./data', train=False, download=True, transform=transform)
    
    # FIX 3: drop_last=True to prevent tiny-batch gradient noise
...
def get_resnet():
    ...
    # FIX 4: Force conv1 to accept 1 channel instead of 3
