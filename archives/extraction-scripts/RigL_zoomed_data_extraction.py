...
def run_physical_autopsy(weights_path):
    ...
    # ---------------------------------------------------------
    # PANEL 2: The Distribution Curves (Zoomed In)
    # ---------------------------------------------------------
    colors = cm.magma(np.linspace(0.2, 0.9, len(layer_names)))
    
    # Calculate a safe X-axis zoom limit (99.5th percentile to drop the extreme outliers)
    flat_degrees = np.concatenate(all_layer_degrees)
    safe_x_max = np.percentile(flat_degrees, 50) 
    
    for i, degrees in enumerate(all_layer_degrees):
        if degrees.max() == 0: continue
        
        # Filter degrees just for the histogram calculation so the bins don't stretch
    ...
    # Apply the physical zoom limit to the graph
    ...
    ax2.set_title('RigL Evolutionary Spread (Zoomed to 90th Percentile)', fontsize=14, fontweight='bold')
