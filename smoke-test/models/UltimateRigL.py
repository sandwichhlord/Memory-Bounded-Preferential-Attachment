...
if IS_COLAB:
    ...
    SAVE_DIR = '/content/drive/MyDrive/Thesis_Ultimate_RigL/'
...
DELTA_T = 100           # Update topology every 100 batches
...
HISTORY_FILE = os.path.join(SAVE_DIR, f'ultimate_rigl_{DATASET.lower()}_history.csv')
DEGREES_FILE = os.path.join(SAVE_DIR, f'ultimate_rigl_{DATASET.lower()}_degrees.csv')
WEIGHTS_FILE = os.path.join(SAVE_DIR, f'ultimate_rigl_{DATASET.lower()}_sparse.pth')
...
# ==========================================
# 2. THE ULTIMATE RigL SCHEDULER
# ==========================================
class UltimateRigLScheduler:
    def __init__(self, model, optimizer, sparsity=0.9, delta_t=100, alpha=0.3, T_end=None, device='cuda'):
        self.model = model
    ...
    def get_growth_fraction(self):
        ...
        return self.alpha * 0.5 * (1 + math.cos(self.iterations * math.pi / self.T_end))
        ...
        with torch.no_grad():
            for name, param in self.model.named_parameters():
                ...
                if num_to_prune > 0:
                    ...
                    # 2. Gradient Growth (Dense)
                    grads = torch.abs(param.grad.data)
                    dead_grads = torch.where(mask == 0, grads, torch.tensor(-1.0).to(self.device))
                    top_grads = torch.topk(dead_grads.flatten(), num_to_prune).indices

                    # 3. Apply New Masks & Zero-Init
                    mask.view(-1)[top_grads] = 1.0
                    param.data.view(-1)[top_grads] = 0.0

                self.masks[name] = mask

                # 4. Clear stale momentum for dead weights
...
# ==========================================
# 3. ARCHITECTURE & DATALOADERS (UPDATED WITH AUGMENTATION)
# ==========================================
def get_dataloaders():
    if DATASET == 'MNIST':
        ...
        # THE FIX: CIFAR-10 DATA AUGMENTATION
...
def main():
    ...
    print(f"--- Ultimate RigL Final Run: {DATASET} on {device} ---")

    train_loader, test_loader = get_dataloaders()
    model = get_resnet().to(device)

    # weight_decay=5e-4 is present. This is the standard regularization needed for ResNets.
    ...
    rigl_scheduler = UltimateRigLScheduler(model, optimizer, sparsity=SPARSITY, delta_t=DELTA_T, alpha=ALPHA, T_end=int(total_iters * 0.8), device=device)
    ...
    final_mask = rigl_scheduler.masks['fc.weight'].cpu()
